...
public class HttpProxy extends Thread {
	...
	public String processHostName(String url, String host, int port, Socket sock) {
		...
		System.out.println("sock.getInetAddress():"+sock.getInetAddress());
		...
		try {
			...
			while (true) {
				try {
					...
					System.out.println("os0:"+os0);
					System.out.println("os1:"+os1);
					System.out.println(is0 instanceof FileInputStream);
//					charsetName = FileCharset.getFileCharset(is0);
...
//							charsetName="GBK";	
//						}
						isr = new InputStreamReader(is0,"GBK");
						osw = new OutputStreamWriter(os0,"UTF-8");
						if ((ir = isr.read(buffer)) > 0) {
							osw.write(buffer, 0, ir);
						} else if (ir < 0)
							break;
//					}
				...
				try {
//					charsetName = FileCharset.getFileCharset(is1);
//					System.out.println("charsetName:"+charsetName);
//					if(charsetName==null){
...
//							charsetName="GBK";
//						}
						isr = new InputStreamReader(is1,"GBK"); 
						osw = new OutputStreamWriter(os1,"UTF-8");
						if ((ir = isr.read(buffer)) > 0) {
							osw.write(buffer, 0, ir);
						} else if (ir < 0)
							break;
